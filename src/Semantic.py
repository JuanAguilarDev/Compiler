txt = " " # Almacenara las cadenas de texto que formaran el arbol semantico
cont = 0 # Esta va a almacenar el numero de veces que se ha traducido una instruccion
def incremetarContador():
	global cont
	cont +=1
	return "%d" %cont

# Se necesita que sean nodos para generar el arbol semantico
class Nodo():
	pass

# Una clase para cada produccion escrita en la clase Syntax
# Tienen atributos en comun pero algunos tienen hijos y otros no, los hijos son los nodos que siguen de la clase
# El contador se incrementa para darle un identificador a cada clase y asi saber cual es el identificador de cada no terminal
# Name es el nombre del nodo que viene del archivo sintactic
# Al ser no terminales, tienen produccion, esas producciones son los hijos que reciben en el constructor
# Imprimir nos ayuda a crear un arbol, primero los hijos y luego los padres (postOrden)
# Se imprime de acuerdo a lo que se genera en el analisis sintactico
# La indentacion es debido a la sintaxis de pl0
class Null(Nodo):
	def __init__(self):
		self.type = 'void'
...
class program(Nodo):
	...
	def imprimir(self,ident):
        # Primero el nombre del hijo, el orden es debido a las producciones
		self.son1.imprimir(" "+ident)
        # Despues el nombre del padre
